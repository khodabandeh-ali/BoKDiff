...
# Add the project root to the Python path for utils
...
def parse_crossdocked_index_file(dataset_dir='./new_data_temp'):
    all_index = []
    # Traverse each folder in the dataset directory
    for folder in os.listdir(dataset_dir):
        folder_path = os.path.join(dataset_dir, folder)
        # Check if it's a directory
        if os.path.isdir(folder_path):
            file_dict = {}
            # Find .pdb and .sdf files in the folder
            for file_name in os.listdir(folder_path):
                if file_name.endswith('.pdb'):
                    file_dict['src_protein_filename'] = f"{folder}/{file_name}"
                elif file_name.endswith('generated_aligned.sdf'):
                    file_dict['src_ligand_filename'] = f"{folder}/{file_name}"
            # Add the dictionary to the list if both files were found
            if 'src_protein_filename' in file_dict and 'src_ligand_filename' in file_dict:
                all_index.append(file_dict)
    return all_index
...
if __name__ == '__main__':
    ...
    if config.dataset.name == 'pdbbind':
        ...
        all_index = parse_crossdocked_index_file(config.dataset.dataset_dir)
    else:
        raise NotImplementedError
