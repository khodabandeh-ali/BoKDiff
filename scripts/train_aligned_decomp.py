...
if __name__ == '__main__':
    ...
    # Logging
    log_dir = misc.get_new_log_dir(args.logdir, prefix=config_name, tag=alignment_iter)
    ...
    dataset = get_decomp_dataset(
        config=config.data,
        transform=transform,
    )
    dataset2, subsets = get_decomp_dataset(
        config=config.data_val,
        transform=transform,
    )
    train_set = dataset
